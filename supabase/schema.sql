...
BEGIN
  ...
  -- FARA filtrare pe destinatie — contorul e global pe range
  -- Daca nu exista colete, default = range_start (primul colet va fi range_start + 1)
  -- Ex: range 0-100 → primul colet = 1, range 100-200 → primul = 101
  ...
  v_next_id := v_max_used + 1;

  IF v_next_id >= v_range_end THEN
    RAISE EXCEPTION 'Range epuizat pentru sofer %. Limita: %', p_driver_id, v_range_end;
  END IF;
